...
import { faUser, faCartPlus, faSearch, faBars, faClose } from '@fortawesome/free-solid-svg-icons';
...
const Navbar = () => {
  ...
  const router = useRouter(); // Ensure useRouter is called
  ...
  return (
    <>
      {/* Desktop Navbar */}
      <div className="navbar bg-slate-500 text-black flex justify-between m-2">
        <div>
          ...
          <div className={`flex-1 justify-self-center pb-3 mt-8 md:block md:pb-0 md:mt-0 ${navbar ? 'p-12 md:p-0 block' : 'hidden'}`}>
            <ul className="h-screen md:h-auto items-center justify-center md:flex">
                ...
                {isMounted && (
                  ...
                  >
                    ...
                    {typeof window !== 'undefined' && cartCount > 0 ? (
                      ...
                      isCartHovered && (
                        <span className="absolute bg-white text-black p-2 rounded shadow-lg text-xs mt-8">Cart is empty</span>
                      )
                    )}
                  </Link>
                )}
              ...
              <li className="pb-4 text-xs text-white py-2 md:px-2 text-center border-b-2 md:border-b-0 hover:bg-purple-900 border-purple-900 md:hover:text-purple-600 md:hover:bg-transparent">
                <button onClick={toggleSearch} className="flex items-center justify-center w-8 h-8 rounded-full bg-rose-400 hover:bg-rose-200 transition duration-200 p-2 ml-4">
                  <FontAwesomeIcon icon={faSearch} className="text-white" />
                </button>
              </li>
              <li className="pb-4 text-xs text-white py-2 md:px-2 text-center border-b-2 md:border-b-0 hover:bg-purple-900 border-purple-900 md:hover:text-purple-600 md:hover:bg-transparent">
                <Link href="/login" className="flex items-center justify-center w-8 h-8 rounded-full bg-rose-400 hover:bg-rose-300 transition duration-200 p-2 ml-4">
                  <FontAwesomeIcon icon={faUser} className="text-white" />
                </Link>
              </li>
              <li className="pb-4 text-xs text-white py-2 md:px-2 text-center border-b-2 md:border-b-0 hover:bg-purple-900 border-purple-900 md:hover:text-purple-600 md:hover:bg-transparent">
                <Link href="/register" className="inline-block px-4 py-1 text-white bg-rose-400 rounded hover:bg-rose-300 transition duration-200 ml-2">
                  SIGN UP
                </Link>
              </li>
            </ul>
